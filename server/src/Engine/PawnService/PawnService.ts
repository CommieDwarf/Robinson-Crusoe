import {IPawnService, IPawnServiceRenderData, PawnArrayName,} from "../../types/Pawns/PawnService";
import {IPawn, IPawnOwner, PAWN_HELPER_ACTION} from "../../types/Pawns/Pawn";
...
export class PawnService<Owner extends IPawnOwner> implements IPawnService<Owner> {
    ...
    public addPawn(disposable: boolean, action: PAWN_HELPER_ACTION | null) {
        const pawn = new Pawn<Owner>(this._owner, disposable, action);
    ...
    public initPawns(initialQuantity: number, disposable: boolean, action: PAWN_HELPER_ACTION | null): void {
