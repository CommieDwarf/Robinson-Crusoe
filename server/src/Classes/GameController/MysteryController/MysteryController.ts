...
export class MysteryController implements GameControllerInterface {
    ...
    public getActionHandlers(): Map<MYSTERY_CONTROLLER_ACTION, ActionHandler> {
        ...
        handlers.set(MYSTERY_CONTROLLER_ACTION.RESOLVE_EVENT_MYSTERY, this.resolveEventMystery.bind(this));
        handlers.set(MYSTERY_CONTROLLER_ACTION.TRIGGER_MYSTERY_DRAW_EFFECT, this.triggerMysteryDrawEffect.bind(this));
        handlers.set(MYSTERY_CONTROLLER_ACTION.USE_TREASURE_CARD, this.useTreasureCard.bind(this));
        handlers.set(MYSTERY_CONTROLLER_ACTION.DRAW_MYSTERY_CARD, this.drawMysteryCard.bind(this));
        handlers.set(MYSTERY_CONTROLLER_ACTION.FINISH_DRAWING_MYSTERY_CARDS, this.finishDrawingMysteryCards.bind(this));
        handlers.set(MYSTERY_CONTROLLER_ACTION.MANAGE_CARD_STORAGE, this.manageCardStorage.bind(this));
        return handlers;
    }
    ...
    private manageCardStorage(player: IPlayer, cardName: string, action: STORAGE_ACTION): void {
        switch (action) {
