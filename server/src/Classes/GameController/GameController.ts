...
export class GameController implements GameControllerInterface, BaseController {
	...
	private readonly _actionHandlers = new Map<
		CONTROLLER_ACTION,
		ActionHandler
	>();
	...
	public loadBySteps(steps: SaveStep[]) {
		steps.forEach((step) => {
			this.handleAction(
				step.action,
				this.getPlayerByUserId(step.userId),
				...step.args
			);
		});
	}
	...
	public getActionHandlers(): Map<OTHER_CONTROLLER_ACTION, ActionHandler> {
		...
		handlers.set(
			OTHER_CONTROLLER_ACTION.ADD_WOOD_TO_PILE,
			this.addWoodToPile.bind(this)
		);
		handlers.set(
			OTHER_CONTROLLER_ACTION.RESOLVE_EVENT_ADVENTURE,
			this.resolveEventAdventure.bind(this)
		);
		handlers.set(
			OTHER_CONTROLLER_ACTION.ROLL_WEATHER_DICES,
			this.rollWeatherDices.bind(this)
		);
		handlers.set(
			OTHER_CONTROLLER_ACTION.SET_NEXT_PHASE,
			this.setNextPhase.bind(this)
		);
		handlers.set(
			OTHER_CONTROLLER_ACTION.SWITCH_COMMITTED_RESOURCES_TYPE,
			this.switchCommittedResourcesType.bind(this)
		);
		handlers.set(
			OTHER_CONTROLLER_ACTION.USE_INVENTION,
			this.useInvention.bind(this)
		);
		handlers.set(OTHER_CONTROLLER_ACTION.USE_ITEM, this.useItem.bind(this));
		handlers.set(
			OTHER_CONTROLLER_ACTION.USE_DISCOVERY_TOKEN,
			this.useDiscoveryToken.bind(this)
		);
		handlers.set(
			OTHER_CONTROLLER_ACTION.PICK_OBJECT,
			this.pickObject.bind(this)
		);
		return handlers;
	}
	...
	private resolveEventAdventure(player: IPlayer, option: 1 | 2): void {
		this._game.eventService.resolveEventAdventure(
			option,
			player.getCharacter()
		);
	}
	...
	private switchCommittedResourcesType(
		player: IPlayer,
	...
	private useInvention(player: IPlayer, inventionName: INVENTION): void {
		this._game.inventionService.useInvention(
			inventionName,
			player.getCharacter()
		);
	}
