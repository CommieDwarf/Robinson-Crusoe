...
export class TileController implements GameControllerInterface {
	...
	getActionHandlers() {
		...
		handlers.set(
			...
			this.moveCamp.bind(this)
		);
		handlers.set(
			TILE_CONTROLLER_ACTION.TRIGGER_TILE_ACTION,
			this.triggerTileAction.bind(this)
		);
		handlers.set(
			TILE_CONTROLLER_ACTION.TRIGGER_TILE_RESOURCE_ACTION,
			this.triggerTileResourceAction.bind(this)
		);
		return handlers;
	}
	...
	private triggerTileResourceAction(
		player: IPlayer,
		tileID: number,
		side: Side
	): void {
