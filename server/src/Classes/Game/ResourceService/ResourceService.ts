...
} from "@shared/types/Game/Resources/AllResources";
...
export class ResourceService implements IResourceService {
	...
	private unpackRenderData(resources: IResources): IResourcesRenderData {
		return {
			...
			treasures: resources.treasures.map(
				(treasure) => treasure.renderData
			),
		};
	}
	...
	public removeTreasureFromFuture(card: IMysteryCard) {
		this._future.treasures = this._future.treasures.filter(
			(c) => c !== card
		);
	}
	...
	public addBasicResourcesToOwned = (
	...
	public addBasicResourcesToFuture(
		resources: IBasicResources,
		logSource: string = ""
	) {
	...
	private spendResourceFromOwned(
		resource: keyof IBasicResourcesAmount,
		amount: number,
		logSource: string
	): void {
		if (!this._owned.basic.canAfford(resource, amount)) {
			throw new Error(
				`Spend amount can't be greater than owned! spendAmount: ${amount}. owned: ${this._owned.basic.getResource(
					resource
				)}`
			);
		}
	...
	public getOwnedTreasureMysteryCard(cardName: string) {
		const card = this._owned.treasures.find(
