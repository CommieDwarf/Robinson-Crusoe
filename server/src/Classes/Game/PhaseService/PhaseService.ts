import {
	...
	Phase,
	PhaseEffects,
} from "@shared/types/Game/PhaseService/PhaseService";
...
export class PhaseService implements IPhaseService {
	...
	public removePhaseEffect(effect: Function) {
		this._gainedPhaseEffects = this._gainedPhaseEffects.filter(
			(func) => func !== effect
		);
	}
	...
	private preActionEffect = () => {
		try {
			const occupiedSlots =
				this._game.actionSlotService.getOccupiedActionSlots();
	...
	private eatOrGetHurt() {
		this._game.playerService.players.forEach((player) => {
			...
			if (this._game.resourceService.canAffordResource("food", 1)) {
				...
				this._game.resourceService.canAffordResource("dryFood", 1)
			) {
				this._game.resourceService.spendBasicResourceIfPossible(
					"dryFood",
					1,
	...
	private rotFood() {
		if (
			!this._game.mysteryService.hasTreasureCard(
