...
} from "@shared/types/Game/Beasts/BeastService";
...
export class BeastService implements IBeastService {
	...
	private resetWeaponBoosts(hunter: ICharacter) {
		this._game.constructionService
			.getConstruction(CONSTRUCTION.WEAPON)
			.resetTemporaryBoost();
	...
	private addHuntReward(beast: IBeast) {
		if (this._game.phaseService.phase === "action") {
			this._game.resourceService.addBasicResourcesToFuture(
				beast.reward,
				ACTION.HUNT
			);
		} else {
	...
	private initBeasts() {
		this._beastStack = Object.values(BEAST).map((beast) =>
			this._beastCreator.create(beast)
		);
	}
}
