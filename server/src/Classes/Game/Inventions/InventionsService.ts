...
} from "@shared/types/Game/InventionService/InventionService";
...
export class InventionsService implements IInventionService {
	...
	get renderData(): IInventionServiceRenderData {
		return {
			inventions: this.inventions.map(
				(invention) => invention.renderData
			),
		};
	}
	...
	private initInventions(scenarioName: "castaways") {
		...
		const personal = this._game.characterService.playerCharacters.map(
			(char) => char.invention
		);

		return [
			...normal,
			...[...starters, ...scenario, ...personal].map((invention) => {
	...
	destroy(name: INVENTION) {
		...
		if (!this._builtInventions.includes(invention)) {
			throw new Error(
				"There is no such invention built: " + invention.name
			);
		}
	...
	public resetCardPawns() {
		this._builtInventions.forEach((card) =>
			card.pawnService?.resetFreePawns()
		);
	}
	...
	getInvention(name: INVENTION): IInvention {
		...
		if (!invention) {
			throw new Error(
				"Can find invention with specific scenario: " + name
			);
		}
		return invention;
	}
