...
export const Resources = (props: Props) => {
  ...
  const resourcesAmount = new Map<keyof AllResources, number>([
    ...(Object.entries(props.resources.basic) as Entries<
      typeof props.resources.basic
    >),
  ...
  resourcesAmount.forEach((value, key) => {
    ...
    resources.push(
      <div className={styles.resource} key={key}>
        ...
        <div
          className={`${styles.value} ${valueChanged ? styles.valueChanged : ""}`}
        >
          {value}
        </div>
      </div>,
  ...
  return (
    ...
    >
      <div className={styles.label}>
        {props.type === "future" ? t("other.future") : t("other.owned")}
      </div>
