...
export default function ChatLog(props: Props) {
	...
	useEffect(() => {
		...
		const chatMessagesChanged =
			prevChatMessages?.length !== chatMessages?.length;
		const logMessagesChanged =
			prevLogMessages?.length !== logMessages?.length;
		...
		if (
			((logMode && logMessagesChanged) ||
				(!logMode && chatMessagesChanged)) &&
			isScrollAtBottom
		) {
			scrollToBottom(true);
		}
	...
	function onSendMsgClick() {
		if (message.trim().length > 0) {
			dispatch(
				socketEmit(SOCKET_EVENT_CLIENT.SEND_MESSAGE, {
	...
	function handleScroll(event: React.UIEvent<HTMLDivElement>) {
		...
		if (
			messagesHeight - scrollHeight <
			event.currentTarget.scrollTop + scrollTolerance
		) {
