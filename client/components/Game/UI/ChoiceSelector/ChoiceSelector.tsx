...
export function ChoiceSelector(props: Props) {
    ...
    const {amount, objects, source, pickSubject, hasSecondEffect} = props.choiceSelector;
    ...
    function chooseObject(isSecondary: boolean) {
        if (selectedObjectIds.length === amount) {
            dispatch(socketEmitAction(OTHER_CONTROLLER_ACTION.PICK_OBJECT, props.choiceSelector.id, selectedObjectIds, isSecondary));
        }
    }
        ...
        <Draggable bounds={"parent"}>
            <div className={styles.container}>
                <span className={styles.source}>
                    <h1>
                        {/*@ts-ignore*/}
                        {useDynamicTranslation(props.choiceSelector.source)}
                    </h1>
                </span>
                {Object.values(ABILITY).includes(props.choiceSelector.source as ABILITY) &&
                    <div className={styles.descriptionWrapper}>
                        <span className={styles.description}>
                        {/*@ts-ignore*/}
                            {insertIconsIntoText(t(`ability.${props.choiceSelector.source}.description`), styles.icon)}
                      </span>
                    </div>

                }

                {props.choiceSelector.pickSubject !== "construction" && <div className={styles.pickObjects}>
