...
export function ActionSlots(props: Props) {
    ...
    >
        {props.tile.canBeExplored && requiredPawnAmount &&
            <div className={`${styles.exploreSlots}
                ${scrollable && styles.scrollable}
                ${scrollable && "preventMapScroll"}`
            }>
                {getActionSlots(props.tile, requiredPawnAmount).map((slot, i) => {
                    return <div className={styles.exploreSlot} key={i}>{slot}</div>
                })}
            </div>
        }
        {props.tile.canBeGathered && requiredPawnAmount && props.tile.tileResourceService &&
            <div className={
                `${styles.gatherSlotsWrapper} 
                  ${scrollable && styles.scrollable}
                 ${scrollable && "preventMapScroll"}`
            }>
                <div
                ...
                >
                    ...
                    {(["left", "right"] as Side[]).map((side, i) => {
                        return <div className={styles.gatherSlotsColumn} key={i}>
                            {props.tile.tileResourceService?.resources[side].canBeGathered &&
                                getActionSlots(props.tile, requiredPawnAmount, side).map((slot, j) => {
                                    return <div className={styles.gatherSlot} key={j}>{slot}</div>
                                })
                            }
                        </div>
                    })}
