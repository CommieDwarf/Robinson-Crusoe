...
export function getCharacterRerollAbility(character: {
	abilities: IAbilityRenderData[];
	name: CHARACTER;
}): IAbilityRenderData | undefined {
	switch (character.name) {
		case CHARACTER.COOK:
			return character.abilities.find(
				(ab) => ab.name === ABILITY.SCROUNGER
			);
		case CHARACTER.FRIDAY:
			return character.abilities.find(
				(ab) => ab.name === ABILITY.FRIDAYS_ABILITY
			);
		case CHARACTER.CARPENTER:
			return character.abilities.find(
				(ab) => ab.name === ABILITY.CRAFTSMANSHIP
			);
