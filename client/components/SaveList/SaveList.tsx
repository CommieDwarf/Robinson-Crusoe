...
export function SaveList(props: Props) {
    ...
    function handleLoadClick() {
        if (selectedSaveId) {
            dispatch(socketEmit(SOCKET_EVENT_CLIENT.LOAD_SAVE, {saveId: selectedSaveId}))
        }
    }

    function handleDeleteClick() {
        if (selectedSaveId) {
            dispatch(socketEmit(SOCKET_EVENT_CLIENT.DELETE_SAVE, {saveId: selectedSaveId}))
        }
    }
    ...
    return <div className={styles.container}>
        ...
        <div className={styles.list}>
            {saveList.map((save, i) => {
                return <SaveOverview save={save} index={i + 1} onClick={selectSave} key={i}
                                     selected={save.id === selectedSaveId}/>
            })}
        </div>
        <div className={styles.buttons}>
            <div className={`${styles.button} menuButton`} onClick={handleLoadClick}>
                Wczytaj
            </div>
            <div className={`${styles.button} menuButton ${styles.buttonDelete}`} onClick={handleDeleteClick}>Usuń</div>
        </div>
    </div>
}
