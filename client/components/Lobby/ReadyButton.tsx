...
export function ReadyButton(props: Props) {
    ...
    return <div className={`${styles.container} ${props.ready && styles.ready}`} onClick={handleClick}>
        <div className={`${styles.readyImg}`}>
            {props.ready && <ResizableImage src={checkMark} alt={"ready"}/>}
            {!props.ready && <ResizableImage src={xMarkImg} alt={"not ready"}/>}
        </div>
        <div className={`${styles.circle} ${!props.ready && styles.notReady}`}>
