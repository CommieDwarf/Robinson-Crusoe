...
export function Icon({ icon }: { icon?: Icon; delimiter?: string }) {
  return <span>{insertIconsIntoText(`$${icon}$`)}</span>;
}
...
export function insertIconsIntoText(
  ...
  return splitByWordAndIcon(string, delimiter)
    .map((str, i) => {
...
function splitByWordAndIcon(string: string, delimiter: string) {
  // Podmieniamy & na &@ aby po splicie zachować informację o ikonie.
  // Podmieniamy " " na " &" aby zachować spacje.
  string = string.replaceAll(delimiter, "$@").replaceAll(" ", " &");
...
function getProperImg(
  ...
  if (icons.includes(str.toLowerCase() as Icon)) {
    ...
    return (
      ...
      >
        <DynamicImage src={`/UI/icons/${str.toLowerCase()}.webp`} alt={str} />
      </div>
    );
  }
